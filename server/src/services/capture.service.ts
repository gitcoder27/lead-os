...
export class CaptureService {
  ...
  async run(input: CaptureRequestBody, principal: TaskPrincipal): Promise<CaptureResponseBody> {
    ...
    switch (resolved.intent) {
      ...
      default:
        return { ...base, task: await this.applyCreate(resolved, principal) };
    }
  }
    ...
    const [peopleRows, jiraRows] = await Promise.all([
      ...
      jiraKeys.length
        ? db.select({ jiraKey: issues.jiraKey }).from(issues)
            .where(and(eq(issues.workspaceId, workspaceId), inArray(issues.jiraKey, jiraKeys)))
        : Promise.resolve([]),
    ]);
  ...
  private async applyCreate(resolved: ResolvedCapture, principal: TaskPrincipal) {
    const scope = normalizeWorkspaceId(principal.workspaceId);
    ...
    const row = await this.taskService.create(
      {
        ...
        followUpAt: resolved.followUp ? `${resolved.followUpAt ?? today}T00:00:00.000Z` : null,
        parentId,
      },
      principal,
    );
